// ─────────────────────────────────────────────────────────────────
//  BANIYA JOBS — All Screens
//  Each screen is exported separately and imported in App.js
// ─────────────────────────────────────────────────────────────────
...
export function BottomNav({ active }) {
  ...
  const items = [
    { id: "home",    path: "/home",    icon: "🏠", label: "Jobs",    labelNp: "जागिर" },
    { id: "saved",   path: "/saved",   icon: "🔖", label: "Saved",   labelNp: "सेभ",   badge: saved.length },
    { id: "post",    path: "/post",    icon: "➕", label: "Post",    labelNp: "पोस्ट" },
    { id: "applied", path: "/saved",   icon: "📋", label: "Applied", labelNp: "आवेदन", badge: applied.length },
    { id: "profile", path: "/profile", icon: "👤", label: "Profile", labelNp: "प्रोफाइल" },
  ];
  return (
...
export function TopBar({ title, backTo, right }) {
  ...
  return (
    <div className="topbar">
      {backTo && (
        <button className="topbar-back" onClick={() => navigate(backTo)}>‹</button>
      )}
  ...
  return (
    <div className="screen">
      ...
      <div style={{ background: "var(--navy)", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "60px 24px 32px", textAlign: "center" }}>
        <div style={{ background: "rgba(244,168,35,0.2)", width: 84, height: 84, borderRadius: 22, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18, fontSize: 38 }}>💼</div>
        ...
        <p style={{ fontSize: 11, color: "rgba(255,255,255,.25)" }}>
          सुरु भयो म्याग्दी, बेनीबाट · नेपालभर
        </p>
      </div>

      {/* Actions */}
      <div style={{ padding: "24px 24px 40px", background: "#fff" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          <div className="card" style={{ textAlign: "center", cursor: "pointer", border: "2px solid var(--border)", padding: "20px 12px" }}
...
export function RegisterScreen() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", district: "", bizName: "", bizType: "", password: "" });
  const { show, Toast } = useToast();

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const register = () => {
    if (!form.name)     return show("Please enter your full name");
    if (!form.phone)    return show("Please enter your phone number");
    if (!form.district) return show("Please select your district");
    if (!form.password || form.password.length < 6) return show("Password must be at least 6 characters");
    if (role === "employer" && !form.bizName) return show("Please enter your business name");

    // TODO: Replace with Firebase auth + Firestore write
    setUser({ name: form.name, phone: form.phone, district: form.district, role, verified: false, bizName: form.bizName });
    navigate("/pending");
  };
      ...
      <div className="scroll">
        ...
        <div className="card" style={{ background: role === "employer" ? "#FFF7ED" : "var(--warm)", border: `1.5px solid ${role === "employer" ? "rgba(244,168,35,.3)" : "var(--border)"}`, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--muted)" }}>
            {role === "employer" ? "Register your business · Post unlimited jobs · Free to start" : "Register free · Get verified · Apply to any job in Nepal"}
          </div>
        </div>
        ...
        <div className="field">
          ...
          <input className="input" placeholder="e.g. Hari Bahadur Thapa" value={form.name} onChange={e => set("name", e.target.value)} />
        </div>
        <div className="field">
          <label className="label">Phone Number / फोन नम्बर</label>
          <input className="input" type="tel" placeholder="+977 98XXXXXXXX" value={form.phone} onChange={e => set("phone", e.target.value)} />
        </div>
        <div className="field">
          <label className="label">Your District / तपाईंको जिल्ला</label>
          <select className="input" value={form.district} onChange={e => set("district", e.target.value)}>
            ...
            {PROVINCES.map(p => (
              <optgroup key={p.id} label={`${p.name} — ${p.np}`}>
                {p.districts.map(d => (
                  <option key={d.id} value={d.id}>{d.np} — {d.en}</option>
                ))}
              </optgroup>
            ))}
          </select>
        ...
        {role === "employer" && (
          <>
            <div className="field">
              <label className="label">Business Name / व्यवसायको नाम</label>
              <input className="input" placeholder="e.g. My Hotel, My Shop, My Farm" value={form.bizName} onChange={e => set("bizName", e.target.value)} />
            </div>
            <div className="field">
              <label className="label">Business Type / व्यवसायको प्रकार</label>
              <select className="input" value={form.bizType} onChange={e => set("bizType", e.target.value)}>
                <option value="">Select type...</option>
                {["Hotel / Restaurant / Lodge", "Retail / Shop / Pasal", "Agriculture / Farm / Kheti",
                  "Transport / Vehicle / Logistics", "IT / Tech / Computer", "Construction / Nirman",
                  "Health / Clinic / Hospital", "School / Education", "Government / Sarkar", "NGO / INGO", "Other"].map(t => (
        ...
        <button className="btn btn-primary btn-full btn-lg" onClick={register} style={{ marginTop: 4 }}>
          Create Free Account
        </button>
        ...
        <p style={{ fontSize: 10, color: "var(--muted)", textAlign: "center", marginTop: 14, lineHeight: 1.6 }}>
          Free forever for job seekers. Your account will be verified by our team within 24 hours before you can apply.
        </p>
      </div>
    </div>
  );
...
export function LoginScreen() {
  ...
  const loginEmployer = () => {
    setUser({ name: "Ram Bahadur Sharma", phone: "+16672897651", district: "myagdi", role: "employer", verified: true, bizName: "BeniDash" });
    navigate("/home");
  };
    ...
    <div className="screen">
      ...
      <div className="scroll" style={{ paddingTop: 32 }}>
        ...
        <div className="field">
          ...
          <input className="input" type="tel" placeholder="+977 98XXXXXXXX" value={phone} onChange={e => setPhone(e.target.value)} />
        </div>
        <div className="field">
          <label className="label">Password</label>
          <input className="input" type="password" placeholder="Your password" value={pass} onChange={e => setPass(e.target.value)} />
        </div>
        <button className="btn btn-primary btn-full btn-lg" onClick={login}>Sign In as Job Seeker</button>
        <div style={{ height: 10 }} />
        <button className="btn btn-gold btn-full btn-lg" onClick={loginEmployer}>Sign In as Employer (Demo)</button>
        <div style={{ textAlign: "center", marginTop: 18, fontSize: 12, color: "var(--muted)" }}>
          No account?{" "}
          <span style={{ color: "var(--sky)", cursor: "pointer", fontWeight: 700 }} onClick={() => navigate("/")}>Register free</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="screen" style={{ justifyContent: "center", alignItems: "center", padding: "32px 24px", textAlign: "center" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.75, maxWidth: 280, marginBottom: 24 }}>
        Our team will review and verify your account within <strong>24 hours</strong>. Once verified, you can apply to jobs!
      </p>
      <div style={{ background: "var(--warm)", borderRadius: 16, padding: 18, width: "100%", marginBottom: 20 }}>
        ...
        {[["📋", "Browse available jobs across Nepal"],["🗺️", "Filter by your district or location"],["💬", "WhatsApp us if it's urgent"]].map(([icon, text]) => (
      ...
      <a href="https://wa.me/16672897651" className="btn btn-wa btn-full btn-lg">💬 WhatsApp Us</a>
    </div>
  );
}

// ══════════════════════════════════════════════════════════════════
//  5. HOME SCREEN — Job Listings
// ══════════════════════════════════════════════════════════════════
    ...
    <div className="screen">
      ...
      <div style={{ background: "var(--cream)", padding: "48px 20px 0", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span
              ...
              onClick={() => show(user?.verified ? "✅ Your account is verified!" : "⏳ Verification in progress — usually 24 hours")}
            >
        ...
        <div style={{ marginBottom: 6 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--muted)", marginBottom: 5, textTransform: "uppercase", letterSpacing: ".06em" }}>📍 Filter by location</div>
        ...
        <div style={{ marginBottom: 4 }}>
          <div className="filter-row">
            {JOB_CATEGORIES.filter(c => c.id !== "govt").map(c => (
      ...
      <div className="scroll">
        {jobs.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-text">No jobs found for this filter.<br />Try "All Nepal" to see more.</div>
          </div>
        ) : (
          <>
    ...
    <div className="job-card" onClick={onClick}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 5 }}>
          ...
          {job.verified && <span className="badge badge-verified" style={{ fontSize: 9 }}>✓</span>}
        </div>
        <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 3 }}>
          👥 {job.applies} applied{isSaved ? " · 🔖 Saved" : ""}
        </div>
      </div>
    </div>
  );
...
export function JobDetailScreen() {
  ...
  if (!job) return <div className="screen"><div className="scroll"><div className="empty-state"><div className="empty-icon">😕</div><div className="empty-text">Job not found.</div></div></div></div>;
  ...
  const handleSave = () => {
    toggleSave(job.id);
    show(isSaved ? "Removed from saved" : "Job saved! 🔖");
  };
  ...
  return (
    <div className="screen">
      ...
      <TopBar
        ...
        backTo="/home"
        right={
          <button onClick={handleSave} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: "#fff" }}>
            {isSaved ? "🔖" : "🔖"}
          </button>
        }
      />
      <div className="scroll" style={{ padding: 0, paddingBottom: 20 }}>
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          {/* Location box */}
          <div style={{ background: "var(--warm)", borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 12, color: "var(--navy)", fontWeight: 600 }}>
            📍 {loc}
          </div>
          ...
          <div style={{ fontSize: 11, color: "var(--muted)", textAlign: "center", marginTop: 10 }}>
            👥 {job.applies} people already applied
          </div>
        </div>
      </div>
    </div>
...
export function PostJobScreen() {
  ...
  const { user } = useApp();
  const { show, Toast } = useToast();
  const [form, setForm] = useState({ title: "", cat: "Hotel / Food", locCustom: "", district: "myagdi", salMin: "", salMax: "", type: "Full-time", desc: "", req: "", phone: "", urgent: false });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const submit = () => {
    if (!form.title)     return show("Please enter a job title");
    if (!form.locCustom) return show("Please enter the exact location with a landmark");
    if (!form.desc)      return show("Please describe the job");
    if (!form.phone)     return show("Please enter a contact phone number");
    if (!user?.verified) return show("⚠️ Your account needs to be verified before posting");
    ...
    <div className="screen">
      ...
      <div className="scroll">
        {!user?.verified && (
          <div className="verify-box">
            ...
            <div>
              ...
              <span style={{ fontSize: 11, color: "#92400E" }}>Your account must be verified before you can post jobs. Usually within 24 hours.</span>
            </div>
          </div>
        )}

        <div className="field"><label className="label">Job Title</label><input className="input" placeholder="e.g. Hotel Cook, Delivery Rider, Farm Worker" value={form.title} onChange={e => set("title", e.target.value)} /></div>
        ...
        <div className="field">
          ...
          <input className="input" placeholder="e.g. Beni Bazaar, near Myagdi Bus Park" value={form.locCustom} onChange={e => set("locCustom", e.target.value)} />
        ...
        <div className="field">
          <label className="label">Salary Range (NPR/month)</label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <input className="input" placeholder="Min e.g. 15000" value={form.salMin} onChange={e => set("salMin", e.target.value)} />
            <input className="input" placeholder="Max e.g. 25000" value={form.salMax} onChange={e => set("salMax", e.target.value)} />
          </div>
        </div>
        ...
        <div className="field"><label className="label">Job Description</label>
          <textarea className="input" rows={4} placeholder="What will this person do? Working hours? Any benefits?" value={form.desc} onChange={e => set("desc", e.target.value)} />
        </div>
        <div className="field"><label className="label">Requirements (one per line)</label>
          <textarea className="input" rows={3} placeholder={"Driving license\n2 years experience\nSmartphone"} value={form.req} onChange={e => set("req", e.target.value)} />
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, padding: 12, background: "var(--warm)", borderRadius: 12 }}>
          <input type="checkbox" id="urgent" style={{ width: 18, height: 18, cursor: "pointer" }} checked={form.urgent} onChange={e => set("urgent", e.target.checked)} />
        ...
        <p style={{ fontSize: 10, color: "var(--muted)", textAlign: "center", marginTop: 10 }}>
          Job will be reviewed by our team before publishing. Usually live within 2–4 hours.
        </p>
      </div>
...
export function SavedScreen() {
  ...
  const JobMiniCard = ({ job, showApplied }) => (
      ...
      <div className="scroll">
        ...
        {tab === "saved" && (
          savedJobs.length === 0
            ...
            : savedJobs.map(j => <JobMiniCard key={j.id} job={j} showApplied={false} />)
        )}
        {tab === "applied" && (
          appliedJobs.length === 0
            ...
            : appliedJobs.map(j => <JobMiniCard key={j.id} job={j} showApplied={true} />)
        )}
      </div>
...
export function ProfileScreen() {
  ...
  const logout = () => {
    show("Logging out...");
    setTimeout(() => { setUser(null); navigate("/"); }, 800);
  };
      ...
      <div className="scroll">
        ...
        <div className="menu-row" onClick={toggleLang}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--muted)" }}>{lang === "en" ? "English — tap to switch to Nepali" : "नेपाली — थिच्नुहोस् English"}</div>
          </div>
          <span className="menu-arrow">›</span>
        </div>
        <div className="menu-row" onClick={logout}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--red)" }}>Log Out</div>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: 20, padding: 16, background: "var(--warm)", borderRadius: 14, border: "1.5px solid var(--border)" }}>
          ...
          <a href="https://wa.me/16672897651" style={{ display: "inline-block", marginTop: 8, fontSize: 11, color: "var(--sky)", fontWeight: 600, textDecoration: "none" }}>
            💬 WhatsApp Support
          </a>
        </div>
      </div>
